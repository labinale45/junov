  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {!game.isNew ? (
          <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[11px] font-medium ${CATEGORY_BADGE_CLASS[game.category]}`}>
            {game.categoryLabel}
          </span>
        ) : null}
      </div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {game.isNew ? (
          <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-medium ${CATEGORY_BADGE_CLASS[game.category]}`}>
            {game.categoryLabel}
          </span>
        ) : null}
      </div>
