          ...
          <Reveal>
            <div className="flex items-center justify-center gap-2 mb-2">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                ...
                {courses.length} learning tracks
              </span>
            </div>
            <div className="flex items-center justify-center gap-4">
              <BrandLogo size={44} className="hidden sm:inline-flex" />
          ...
          {courses.map((course, i) => (
            <Reveal key={course.id} delay={i * 0.08}>
              <Link href={`/course/${course.id}`} className="group block h-full">
                <Card className="h-full transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_16px_36px_-12px_rgba(99,102,241,0.35)] hover:border-primary/40">
                  <CardHeader className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className={`p-3 rounded-lg ${course.bg} transition-transform duration-300 ease-out group-hover:scale-110`}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button className="w-full group-hover:bg-primary/90 mt-2" size="sm">
                      Start Learning <ArrowRight className="ml-1 h-4 w-4 transition-transform duration-200 ease-out group-hover:translate-x-1" />
                    </Button>
                  </CardContent>
                </Card>
              </Link>
